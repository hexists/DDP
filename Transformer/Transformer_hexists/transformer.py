...
def point_wise_feed_forward_network(params, x):
    d_model, dff = params['n_hidden'], params['dff']
    output = tf.layers.dense(x, dff, activation=tf.nn.relu)
    return tf.layers.dense(output, d_model, activation=tf.nn.relu)
...
def multi_head_attention(params, v, k, q, mask):
    ...
    wq = tf.layers.dense(q, d_model, activation=None, reuse=tf.AUTO_REUSE)
    wk = tf.layers.dense(k, d_model, activation=None, reuse=tf.AUTO_REUSE)
    wv = tf.layers.dense(v, d_model, activation=None, reuse=tf.AUTO_REUSE)
...
def encoder_layer(params, x, mask):

    num_heads, dff, dropout_rate = params['num_heads'], params['dff'], params['dropout_rate']

    # (batch_size, input_seq_len, d_model)
    attn_output, _ = multi_head_attention(params, x, x, x, mask)
    attn_output = tf.layers.dropout(attn_output, dropout_rate)
    # (batch_size, input_seq_len, d_model)
    out1 = tf.contrib.layers.layer_norm(x + attn_output)

    # (batch_size, input_seq_len, d_model)
    ffn_output = point_wise_feed_forward_network(params, out1)
    ffn_output = tf.layers.dropout(ffn_output, dropout_rate)
...
def decoder_layer(params, x, enc_output, look_ahead_mask, padding_mask):
    dropout_rate = params['dropout_rate']
    # enc_output.shape == (batch_size, input_seq_len, d_model)
    
    # (batch_size, target_seq_len, d_model)
    attn1, attn_weights_block1 = multi_head_attention(params, x, x, x, look_ahead_mask)
    attn1 = tf.layers.dropout(attn1, dropout_rate)
    out1 = tf.contrib.layers.layer_norm(attn1 + x)
    
    # (batch_size, target_seq_len, d_model)
    attn2, attn_weights_block2 = multi_head_attention(params, enc_output, enc_output, out1, padding_mask)
    attn2 = tf.layers.dropout(attn2, dropout_rate)
    # (batch_size, target_seq_len, d_model)
    out2 = tf.contrib.layers.layer_norm(attn2 + out1)
    
    # (batch_size, target_seq_len, d_model)
    ffn_output = point_wise_feed_forward_network(params, out2)
    ffn_output = tf.layers.dropout(ffn_output, dropout_rate)
    # (batch_size, target_seq_len, d_model)
...
class Transformer:
    def __init__(self, params):
        ...
        self.embedding_size = params['n_hidden']
        # input embedding size
        self.num_heads = params['num_heads'] = 8
        self.dff = params['dff'] = self.embedding_size * 4
        self.num_layers = params['num_layers'] = 4
        ...
        with tf.variable_scope('encoder'):
            ...
            # normalize
            self.enc_input_embeddings *= tf.math.sqrt(tf.cast(self.embedding_size, tf.float32))

            # get positional_encoding
            self.enc_pos_encoding = positional_encoding(self.inp_max_len, self.embedding_size)[:, :self.inp_max_len, :]

            # add positional_encoding
            self.enc_input_embeddings += self.enc_pos_encoding

            # dropout
            self.enc_input_embedings = tf.layers.dropout(self.enc_input_embeddings, self.dropout_rate)

            # output
            self.enc_output = self.enc_input_embedings
            self.enc_mask = self.enc_padding_mask
            for i in range(self.num_layers):
                self.enc_output = encoder_layer(self.params, self.enc_output, self.enc_mask)
        ...
        with tf.variable_scope('decoder'):
            ...
            # normalize
            self.dec_input_embeddings *= tf.math.sqrt(tf.cast(self.embedding_size, tf.float32))

            # get positional_encoding
            self.dec_pos_encoding = positional_encoding(self.out_max_len, self.embedding_size)[:, :self.out_max_len, :]

            # add positional_encoding
            self.dec_input_embeddings += self.dec_pos_encoding

            # dropout
            self.dec_input_embedings = tf.layers.dropout(self.dec_input_embeddings, self.dropout_rate)

            # output
            self.dec_output = self.dec_input_embedings
            self.dec_look_ahead_mask = self.combined_mask
            self.dec_padding_mask = self.dec_padding_mask
            self.attention_weights = {}
            for i in range(self.num_layers):
                self.dec_output, self.block1, self.block2 = decoder_layer(self.params, self.dec_output, self.enc_output, self.dec_look_ahead_mask, self.dec_padding_mask)
        ...
        with tf.variable_scope('final'):
            self.model = tf.layers.dense(self.dec_output, self.out_voc_size, activation=None, reuse=tf.AUTO_REUSE)
